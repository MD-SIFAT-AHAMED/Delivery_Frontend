...
const DashboardLayout = () => {
  ...
  >
    ...
    <div className="py-[8px] border-b border-base-300 flex items-center justify-center">
      {isSidebarOpen ? <ProFastLogo /> : <img className="w-9" src={logoImg} alt="" />}
    </div>

    {/* Menu */}
    <nav className="flex-1 px-2 py-4 space-y-2 overflow-y-auto">
      <Link to="/" className="flex items-center gap-3 p-2 rounded hover:bg-base-300 transition">
        <FaHome size={18} /> {isSidebarOpen && <span>Dashboard</span>}
      </Link>

      <Link to="/parcels" className="flex items-center gap-3 p-2 rounded hover:bg-base-300 transition">
        <FaBox size={18} /> {isSidebarOpen && <span>Parcels</span>}
      </Link>

      <Link to="/riders" className="flex items-center gap-3 p-2 rounded hover:bg-base-300 transition">
        <FaMotorcycle size={18} /> {isSidebarOpen && <span>Riders</span>}
      </Link>

      <Link to="users" className="flex items-center gap-3 p-2 rounded hover:bg-base-300 transition">
        <FaUser size={18} /> {isSidebarOpen && <span>Users</span>}
      </Link>

      <Link to="/settings" className="flex items-center gap-3 p-2 rounded hover:bg-base-300 transition">
  ...
  >
    ...
    <header className="navbar bg-base-200 px-4 fixed top-0 z-10">
      <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="btn btn-ghost btn-square">
